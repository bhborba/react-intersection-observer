...
/**
 * For managing the width change, we maintain separate open and closed state 
 * widths. This approach allows us to dynamically adjust the width of each nav 
 * link, enhancing the visual feedback as the user scrolls through the sections.
 */
...
function App() {
  ...
  // It returns a ref and a state inView indicating whether the element is in 
  // view.
  const { ref, inView } = useInView({
    // percentage of visibility before triggering
    threshold: 0.2,
  });
  ...
  const setInView = (inView: boolean, entry: IntersectionObserverEntry) => {
    if (inView) {
      setVisibleSection(entry.target.getAttribute("id") ?? sections[0]);
    }
  };
  ...
  return (
    <div className="flex flex-col pb-12 bg-gray-950">
      ...
      {/** Assign the ref to the DOM element you want to monitor, which in our 
       * case is the section wrapper element. */}
      <div id="section-wrapper" ref={ref}>
        {sections.map((section) => (
          // we use the InView component from react-intersection-observer for 
          // detecting the section in view.

          // This approach allows us to specify the component once within the 
          // map method, rather than invoking the hook four times 
          // (once for each section).

          // onChange: (a callback function for when the in-view state changes)
          // threshold: (a number between 0 and 1 indicating the percentage that 
          // should be visible before triggering)
